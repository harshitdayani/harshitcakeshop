...
const CakeDetails = (props) => {
    ...
    useEffect(() => {
        ...
        axios({
            ...
            console.log("Cake detail : ", response.data);
    ...
    if (cakeDetail.ingredients.length > 0) {
        Ingredients = cakeDetail.ingredients.map((ingredient) => (
            <em>
                <span className="" data-toggle="tooltip" title={ingredient}>{ingredient}</span>
            </em>
        ));
    } else {
